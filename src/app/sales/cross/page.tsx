...
export default function CrossPage() {
  ...
  // Get axis value for a slip/patient
  ...
  const singleResults = useMemo(() => {
    ...
    slips.forEach(s => {
      const key = getAxisValue(rowAxis, s)
      if (!map[key]) map[key] = { count: 0, revenue: 0, patients: new Set() }
      map[key].count++
      map[key].revenue += s.total_price || 0
      if (s.patient_id) map[key].patients.add(s.patient_id)
    })
  ...
  }, [slips, patients, rowAxis, viewMode])

  // === Matrix results ===
  const matrixData = useMemo(() => {
    if (viewMode !== 'matrix') return { rows: [] as string[], cols: [] as string[], cells: {} as Record<string, Record<string, CellData>>, rowTotals: {} as Record<string, CellData>, colTotals: {} as Record<string, CellData>, grandTotal: { count: 0, revenue: 0, patients: new Set<string>() } as CellData }
    ...
    // Sort rows and cols by total revenue
  ...
  }, [slips, patients, rowAxis, colAxis, viewMode])
  ...
  // Heatmap color for matrix cells
  ...
  const totalCount = singleResults.reduce((s, r) => s + r.count, 0)
  const totalRevenue = singleResults.reduce((s, r) => s + r.revenue, 0)
    ...
    <AppShell>
      <div className="max-w-6xl mx-auto px-4 py-4">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-1">
            <button onClick={() => setViewMode('single')}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium ${viewMode === 'single' ? 'bg-[#14252A] text-white' : 'bg-gray-100 text-gray-500'}`}>
              単一軸
            </button>
            <button onClick={() => setViewMode('matrix')}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium ${viewMode === 'matrix' ? 'bg-[#14252A] text-white' : 'bg-gray-100 text-gray-500'}`}>
              2軸マトリクス
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 mb-4">
          <div className="mb-3">
            <span className="text-xs text-gray-500 font-medium mr-2">{viewMode === 'matrix' ? '行軸:' : '集計軸:'}</span>
          ...
          <>
                ...
                {singleResults.length > 0 && (
                  <div className="bg-white rounded-xl shadow-sm p-4 mb-4">
                    <h3 className="text-sm font-bold text-gray-700 mb-3">
                      {axisOptions.find(a => a.key === rowAxis)?.label}別 売上構成
                    </h3>
                    <div className="space-y-2">
                      {singleResults.slice(0, 12).map((r, i) => (
                        <div key={r.label} className="flex items-center gap-2">
                          <span className="text-xs font-medium w-24 sm:w-32 truncate text-right">{r.label}</span>
                          <div className="flex-1 h-6 bg-gray-100 rounded overflow-hidden">
                            <div
                              className="h-full rounded flex items-center px-2"
                              style={{
                                width: `${Math.max(totalRevenue > 0 ? (r.revenue / totalRevenue) * 100 : 0, 1)}%`,
                                backgroundColor: '#14252A',
                                opacity: 1 - (i * 0.05),
                              }}
                            >
                              {r.revenue > 0 && (
                                <span className="text-[10px] text-white whitespace-nowrap">{r.revenue.toLocaleString()}円</span>
                              )}
                            </div>
                          </div>
                          <span className="text-[10px] text-gray-400 w-12 text-right">{totalRevenue > 0 ? Math.round(r.revenue / totalRevenue * 100) : 0}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-gray-50 border-b">
                          <th className="text-left px-3 py-2 text-xs text-gray-500">{axisOptions.find(a => a.key === rowAxis)?.label}</th>
                      ...
                      <tbody>
                        {singleResults.length === 0 ? (
                          ...
                          <tr key={r.label} className="border-b hover:bg-gray-50">
                            ...
                            <td className="px-3 py-2 text-right">{r.patients}人</td>
              ...
              <>
                {matrixData.rows.length === 0 ? (
                  ...
                  <>
                    ...
                    {/* Mobile: stacked cards for matrix */}
                    <div className="sm:hidden space-y-3">
                      {matrixData.rows.slice(0, 15).map(row => (
                        <div key={row} className="bg-white rounded-xl shadow-sm p-3">
                          ...
                          <div className="grid grid-cols-2 gap-1">
                            {matrixData.cols.slice(0, 8).map(col => {
            ...
            {!loading && slips.length > 0 && (
              <div className="bg-blue-50 rounded-xl p-4 mt-4">
                <h3 className="text-sm font-bold text-gray-700 mb-2">使い方のヒント</h3>
                <div className="space-y-1 text-xs text-gray-600">
                  <p>- <strong>単一軸</strong>: 1つの切り口で件数・売上・構成比を比較</p>
                  <p>- <strong>2軸マトリクス</strong>: 2つの軸を掛け合わせて複合分析（例: 年齢層 × 来院経路）</p>
                  <p>- 表示値を切り替えて「件数」「売上」「平均単価」「患者数」を比較可能</p>
                  <p className="mt-1 text-gray-400">※ 年齢層は生年月日から自動計算。未入力は「不明」に分類されます</p>
                </div>
              </div>
            )}
          </>
